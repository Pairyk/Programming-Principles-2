...
# -------------- Initialize before importing! ---------------|
pygame.init()
WIDTH  = 400
HEIGHT = 600
screen = pygame.display.set_mode((WIDTH, HEIGHT))

# -------------- Rest of the importing ---------------|
import random
...
from racer import (Player, EnemyCar, Coin, Obstacle, PowerUp, NitroStrip,
                   draw_road, draw_hud, WIDTH, HEIGHT, ROAD_IMAGE)


# -------------- Difficulty presets ---------------|
DIFF_SETTINGS = {
    'easy': {'enemy_count': 2, 'obstacle_count': 1, 'coin_count': 3, 'base_scroll': 4},
    'normal': {'enemy_count': 3, 'obstacle_count': 2, 'coin_count': 2, 'base_scroll': 6},
    'hard': {'enemy_count': 5, 'obstacle_count': 3, 'coin_count': 1, 'base_scroll': 8},
}


# -------------- Creating all fonts in one place ---------------|
def make_fonts():
    return {
...
def run_game(screen, settings, username, fonts):
    ...
    # --------- sprite groups to launch them all at once ---------|
    ...
    # --------- spawn of objects based on the difficulty ---------|
    # --------- spawn of enemies ---------
    ...
    while running:
        ...
        stripe_offset += diff['base_scroll']

        if stripe_offset >= img_height:
            # Reset to 0, but keep the remainder if the speed is high
            # (e.g. if offset is 605 and height is 600, reset to 5 for smoothness)
            stripe_offset %= img_height
